...
>
>**上传规则:**
>>**GitHub平台上传规则为`单个上传文件小于25MB`和`单次上传文件总数小于100个`。**
>
>**命名规则:**
>>**文件命名:**
>>
...
>>**文件夹/目录命名:**
>>
>>>**根目录命名`[课程标准名称]`, 根目录下级子目录命名`[课程名称+资源类型+(教师姓名)]`的命名规则进行归档。参考书籍/参考答案请直接放在根目录中, 不需要进行分类。**
>
>**课程结构示意图**
...
>
>**资源年限:**
>>**上传的资源是`近 5 年内`的。年代久远的资料并不能发挥太大作用。如果材料足够优质, 上传请针对文件进行标记说明, 防止PR请求被拒绝。**
>
...
**由于本项目体积很大，故推荐采用在 **GitHub Web 端直接上传** 的方式，具体操作如下：**
    ...
    2. 上传文件 / 文件夹
       ...
       ii) 上传文件到新文件夹：
            按照`特别注意!!`中提到的`课程结构示意图`在本地新建一个文件夹后, 进入仓库的主页面中。点击Upload files，上传你的文件夹。
